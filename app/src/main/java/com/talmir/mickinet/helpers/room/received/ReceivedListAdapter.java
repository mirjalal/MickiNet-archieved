...
public class ReceivedListAdapter extends RecyclerView.Adapter<ReceivedListAdapter.ReceivedViewHolder> {
    ...
    class ReceivedViewHolder extends RecyclerView.ViewHolder {
        private final TextView receivedItemView;

        public ReceivedViewHolder(View itemView) {
            super(itemView);
            receivedItemView = itemView.findViewById(R.id.textView);
        }
    }

    private final LayoutInflater mInflater;
    private List<Received> mReceiveds; // Cached copy of receiveds

    public ReceivedListAdapter(Context context) { mInflater = LayoutInflater.from(context); }
     ...
     */
    @Override
    public void onBindViewHolder(ReceivedViewHolder holder, int position) {
        if (mReceiveds != null) {
            Received current = mReceiveds.get(position);
            holder.receivedItemView.setText(current.fileName);
        } else {
            // Covers the case of data not being ready yet.
            holder.receivedItemView.setText("No file name");
        }
    }
    ...
    public int getItemCount() {
        return mReceiveds != null ? mReceiveds.size() : 0;
    }
}
